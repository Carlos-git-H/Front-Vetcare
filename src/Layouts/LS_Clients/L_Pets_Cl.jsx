...
import ModalNewPet_Cl from '../../Components/CS_Clients/ModalNewPet_Cl';
...
function L_Pets_Cl() {
    ...
    useEffect(() => {
        ...
        const fetchPets = async () => {
            try {
                const response = await fetch(`http://localhost:8080/api/pets/client?clientId=${userId}`);
                if (!response.ok) {
    ...
    const handleBlockPet = (petId) => {
        fetch(`http://localhost:8080/api/pets/${petId}/block`, { method: 'PUT' })
            .then((response) => {
                if (!response.ok) {
                    throw new Error("Error al bloquear la mascota.");
                }
            ...
            .catch(() => alert("Error al intentar bloquear la mascota."));
    };
    ...
    const handleUpdate = () => {
        ...
        fetch(`http://localhost:8080/api/pets/client?clientId=${userId}`)
            .then(response => response.json())
            .then(data => setPets(data.content || []))
            .catch(() => alert("Error al actualizar la lista de mascotas."));
    };
                            ...
                            {pets.length > 0 ? (
                                pets.map((pet) => (
                                    <div key={pet.idPet} className="pet-card">
                                        <img src={`/Img/${pet.race.especie.name}.webp`} alt={`Foto de ${pet.name}`} />
                                        <h3>{pet.name}</h3>
